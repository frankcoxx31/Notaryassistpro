...
const SegmentsView: React.FC<SegmentsViewProps> = ({ user, autoOpen }) => {
  ...
  const handlePrintEnvelopes = (e: React.MouseEvent, segment: MarketingSegment) => {
    ...
    const withAddress = members.filter(m => m.address && m.city && m.state);
    const missing = members.length - withAddress.length;

    if (withAddress.length === 0) {
      alert(`None of the ${members.length} contact(s) in "${segment.name}" have a mailing address on file. Add an address on their subscriber record first.`);
      return;
    }
...
<head>
<meta charset="utf-8"/>
<title>Envelopes — ${escapeHtml(segment.name)}</title>
<style>
