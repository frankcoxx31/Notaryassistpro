...
import { X, FileText, Layout, Palette, Code, Loader2, Save } from 'lucide-react';
...
interface EditTemplateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (id: string, updates: Partial<MarketingTemplate>) => Promise<void>;
  template: MarketingTemplate | null;
}

const EditTemplateModal: React.FC<EditTemplateModalProps> = ({ isOpen, onClose, onSave, template }) => {
      ...
      >
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center gap-3">
          ...
          >
            ...
            <span>{loading ? 'Updating...' : 'Update Template'}</span>
          </button>
        </div>
      </motion.div>
    </div>
