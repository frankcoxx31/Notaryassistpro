...
/** Client and appointment fields shared by every template. */
...
/** Per-signature pricing: statutory fee × count, plus mileage. */
const PER_SIGNATURE_FEE_FIELDS = [
...
/**
 * Flat pricing: one agreed fee for the whole job. Used for loan signings,
 * which are billed as a flat signing-agent fee, not per stamp plus mileage.
 */
...
const COMMON_FIELDS = [...APPOINTMENT_FIELDS, ...PER_SIGNATURE_FEE_FIELDS];

/** Opening clauses common to every template. */
const BASE_CLAUSES_INTRO = [
  {
...
/** Fee clauses for per-signature pricing (statutory fee plus mileage). */
const PER_SIGNATURE_FEE_CLAUSES = [
  {
    heading: 'Fees',
    body:
...
/** Full shared clause set for per-signature templates (intro + fees + tail). */
const BASE_CLAUSES = [...BASE_CLAUSES_INTRO, ...PER_SIGNATURE_FEE_CLAUSES, ...BASE_CLAUSES_TAIL];

/** Shared clause set for flat-fee templates (intro + flat fee + tail). */
const FLAT_BASE_CLAUSES = [...BASE_CLAUSES_INTRO, FLAT_FEE_CLAUSE, ...BASE_CLAUSES_TAIL];
...

  {
    ...
    clauses: [
      ...
      },
      {
        ...
        body:
          'If the signing cannot be completed because documents have not arrived, arrive incomplete, contain errors requiring lender correction, or a signer is unavailable or unidentifiable, the appointment will be rescheduled and any agreed trip or print fee remains payable for the trip made.',
      },
      ...FLAT_BASE_CLAUSES,
    ],
    acknowledgements: [
      ...
      ...BASE_ACKNOWLEDGEMENTS.filter(a => a.key !== 'feesUnderstood'),
      {
        key: 'feesUnderstood',
        label:
          'I have reviewed the flat signing fee shown above, I agree to that amount as the total for this signing, and I understand there is no separate per-signature or mileage charge.',
        required: true,
      },
      {
        key: 'lenderQuestions',
...
const CURRENCY_FIELDS = new Set(['notarialFee', 'travelFee', 'flatFee']);
...
export interface ConsentCostBreakdown {
  ...
  travelMiles: number;
  /** The single agreed fee, for flat-fee templates. */
  flatFee: number;
  total: number;
}
 ...
 */
export function computeConsentCost(
  fields: Record<string, string>,
  model: ConsentPricingModel = 'per-signature',
): ConsentCostBreakdown {
  ...
  const total = model === 'flat'
    ? flatFee
    : Math.round((notarialSubtotal + travelFee) * 100) / 100;
...
export function renderConsentDocument(opts: {
  ...
  // otherwise a waived appointment would still read as owing travel. Flat forms
  // get a default for what the fee covers so the clause never reads "not specified".
  const clauseFields = {
    ...fields,
  ...
  const costRows = cost.model === 'flat'
    ? costRow(
        'Flat signing fee',
  ...
  const costNote = cost.model === 'flat'
    ? `This is a flat fee for the signing, payable at the time of service. There is no separate per-signature or mileage charge.`
