...
export const printInvoice = (appointment: Appointment, profile: BusinessProfile | null) => {
  ...
  const bizName = profile?.companyName || 'Integrity Closings CLT';
  const bizEmail = profile?.email || 'notary@integrityclosings.com';
  const bizPhone = profile?.phone || '(704) 555-0123';
  const bizAddress = profile?.address || 'Charlotte, NC';
    ...
    <html>
      ...
      <body>
        <div class="invoice-container">
          ...
          <div class="footer">
            Professional Notary Services | Integrity Closings CLT | Generated by NotaryPro
          </div>
        </div>
      </body>
    </html>
