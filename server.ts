...
const APP_URL = process.env.APP_URL || "https://crm.integrityclosingsclt.com";
        ...
        <tr>
          <td style="background:#f8fafc;padding:24px 40px;border-top:1px solid #e2e8f0;text-align:center;">
            ...
            <p style="margin:6px 0 0;color:#64748b;font-size:12px;">
              <a href="tel:+17045551234" style="color:#2563eb;text-decoration:none;">Call or Text</a>
              &bull;
  ...
  appointment_reminder: {
    ...
    html: (d) => baseTemplate(`
      ...
      <p style="color:#475569;line-height:1.7;margin:0 0 16px;">
        ...
        Do not sign any documents before the appointment — all signatures must be made in the notary's presence.
      </p>
...
async function startServer() {
  ...
  // Send a single email to one customer
  app.post("/api/email/send-single", async (req, res) => {
    ...
    try {
      ...
      if (templateId && TEMPLATES[templateId]) {
        ...
        // Custom compose — wrap in base template
  ...
  // Bulk send to a recipient group from Firestore /customers
  app.post("/api/email/send-newsletter", async (req, res) => {
    ...
    const {
      ...
      body,
      templateData,
      recipientGroups, // array: ['all'] | ['Loan Officer'] | ['Closing Attorney'] | ['General Client'] etc
    } = req.body;
    ...
    try {
      ...
      let query: FirebaseFirestore.Query = adminDb.collection('customers').where('userId', '==', userId);

      // Filter unsubscribed
      // We fetch all then filter in memory to avoid composite index requirements
      ...
      // Filter out customers with no email
  ...
  // GET /api/email/unsubscribe/:customerId
  // One-click unsubscribe — marks customer in Firestore
